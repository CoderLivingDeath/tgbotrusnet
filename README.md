...
reset-db.bat
# Или напрямую: npm run bot db:init -- --force

# Запустить бота
start-server.bat

# Режим отладки (с командой /debug):
start-debug.bat
```

**Все bat-файлы находятся в корне проекта:**

| Файл | Назначение |
|------|-----------|
| `start-server.bat` | Сборка + запуск бота |
| `start-debug.bat` | Сборка + запуск с флагом `-d` |
| `reset-db.bat` | Сброс базы данных (с подтверждением) |
